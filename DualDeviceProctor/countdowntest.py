...
class MotionDetection(object):
    ...
    ##assigning values
    def __init__(self, path, video_source, video_size, frame_rate, threshold, time_interval, recording_time, show_camera, show_mask, debug, examtime):
...
def gen(camera):
    ...
    # MotionDetection(path, video_source =0 , video_size = 720p, frame_rate =24 , threshold = 70000, time_interval 4 seconds before it starts motion detecting again, recording_time =10 , show_camera, show_mask, debug)
        md= MotionDetection("C:\\xampp\\htdocs\\DualDeviceProctor\\videostorage", 0, '720p', 24.0, 70000, 4, 10, False, False, False,4)
        md.start()
        md.end()
